    ...
    if jmessage["event"] == "data":
        ...
        if "isLimitUpPrice" in respdata:  # 最後成交價為漲停價
            if "isTrial" not in respdata:  # 非試搓
                if (
                    respdata["symbol"] not in isLimitUpPriceList
                ):  # 不清單中才加入，且寄送通知
                    isLimitUpPriceList.append(respdata["symbol"])
                    # 寄送漲停通知信
                    SendGmail(
                        getenv("MAILTO"),
                        "[C10 Stock]{0}=最後成交價為漲停價，時間：{1}".format(
                            respdata["symbol"], StockLib.getNowDatetime()
                        ),
                        "",
                    )
...
try:
    with dbinst.getsession()() as session:
        ...
        articles = (
            ...
            .filter(StockInfoType.infotype == sinfotype, StockInfoType.remark == remark)
            .all()
        )
...
stock.on("disconnect", handle_disconnect)
stock.connect()
stock.subscribe({"channel": "trades", "symbols": mySymbols})
