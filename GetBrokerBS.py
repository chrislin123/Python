...
try:
    ...
    with dbinst.getsession()() as session:
        ...
        # 取得所有卷商分點資料
        StockBroker1s = (
            session.query(StockBroker1).filter(StockBroker1.brokerparentyn == "N").all()
        )
        ...
        # 取得本日已經執行完成的分點
        StockBrokerRunOk = (
            session.query(StockLog)
            .filter(
                StockLog.logtype == sLogType,
...
try:
    for StockBrokerData in StockBroker1s:
        ...
        print(
            "{0}/{1}--[{2}]{3}".format(
                idx,
                len(StockBroker1s),
                StockBrokerData.brokercode,
                StockBrokerData.brokername,
            )
        )
        ...
        TWSE_URL = (
            "https://newjust.masterlink.com.tw/z/zg/zgb/zgb0.djhtm?a={0}&b={1}&c=E&d=1"
        )
        TWSE_URL = TWSE_URL.format(
            StockBrokerData.brokerparentcode, StockBrokerData.brokercode
        )
        header = {
            "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36",
        }
        ...
        for table in tables:
            ...
            if "買超" in table.text:
                ...
                for tr in table.findAll("tr"):
                    ...
                    allbuys.append(
                        [stockcode, stockname, stockbuy, stocksell, stockdiff]
                    )
    ...
    SendGmail(
        getenv("MAILTO"),
        "[C10 Stock]每日卷商分點買賣張數取得錯誤資訊={0}".format(
...
try:
    ...
    with dbinst.getsession()() as session:
        ...
        if StockBrokerTotalCount == StockBrokerRunOkCount:
            ...
            Stocklog = (
                session.query(StockLog)
                .filter(
                    StockLog.logtype == "StockBrokerBSDaily",
                    StockLog.logdate == DataDate,
                )
                .first()
            )
            if Stocklog is None:
                ...
                SendGmail(
                    getenv("MAILTO"),
                    "[C10 Stock]{0}=每日卷商分點買賣張數取得完成".format(DataDate),
                    "完成時間：{0} \n分點總數：{1} \n完成數量：{2}".format(
                        StockLib.getNowDatetime(),
                        StockBrokerTotalCount,
                        StockBrokerRunOkCount,
                    ),
                )
                PushMessageEarn2(
                    "[C10 Stock]{0}=每日卷商分點買賣張數取得完成\n完成時間：{0}".format(
                        DataDate, StockLib.getNowDatetime()
                    )
                )
        else:
            # 總數不相同
    ...
    SendGmail(
        getenv("MAILTO"),
        "[C10 Stock]每日卷商分點買賣金額取得錯誤資訊={0}".format(
            StockLib.getNowDatetime()
        ),
        f"Encounter exception: {e}",
    )
