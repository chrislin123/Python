...
try:
    ...
    with dbinst.getsession()() as session:
        ...
        #取得本日已經執行完成的分點
        StockBrokerRunOk = session.query(StockLog).filter(StockLog.logtype == sLogType
...
try:
    for StockBrokerData in StockBroker1s:
        ...
        if "停" in StockBrokerData.brokername:
            #寫入紀錄該分點轉檔已完成
            Stocklog = session.query(StockLog).filter(StockLog.logtype == sLogType
                                                    ,StockLog.logdate == DataDate
...
try:
    ...
    with dbinst.getsession()() as session:
        ...
        if StockBrokerTotalCount == StockBrokerRunOkCount:
            ...
            #寫入每日完成紀錄
            Stocklog = session.query(StockLog).filter(StockLog.logtype == 'StockBrokerBSDaily'
                                                    ,StockLog.logdate == DataDate).first()
            if Stocklog is None:
                Stocklog = StockLog()
