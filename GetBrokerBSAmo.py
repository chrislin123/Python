...
try:
    for StockBrokerData in StockBroker1s:
        ...
        TWSE_URL = (
            "https://newjust.masterlink.com.tw/z/zg/zgb/zgb0.djhtm?a={0}&b={1}&c=B&d=1"
        )
        ...
        with dbinst.getsession()() as session:
            ...
            for data in allbuys:
                ...
                article = (
                    session.query(StockBrokerBSAmo)
                    .filter(
    ...
    with dbinst.getsession()() as session:
        ...
        if StockBrokerTotalCount == StockBrokerRunOkCount:
            ...
            if Stocklog is None:
                ...
                SendGmail(
                    getenv("MAILTO"),
                    "[C10 Stock]{0}=每日卷商分點買賣金額取得完成".format(DataDate),
                    "完成時間：{0} \n分點總數：{1} \n完成數量：{2}".format(
                        StockLib.getNowDatetime(),
