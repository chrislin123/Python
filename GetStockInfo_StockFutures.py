...
try:
    ...
    with dbinst.getsession()() as session:
        ...
        sql = text(
            " update StockInfoType set  updstatus = :updstatus where infotype = :infotype "
        )
        ...
        for data in df.values:
            ...
            article = (
                session.query(StockInfoType)
                .filter(StockInfoType.stockcode == str(data[0]))
                .first()
            )
            if article == None:
        ...
        sql = text(
            " update StockInfoType set status = :status where updstatus = :updstatus "
        )
        session.execute(sql, params)
        session.commit()
...
try:
    ...
    with dbinst.getsession()() as session:
        ...
        for row in tses:
            ...
            article = (
                session.query(stockinfo).filter(stockinfo.stockcode == row[0]).first()
            )
            if article == None:
                article = stockinfo()
