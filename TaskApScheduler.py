...
# 啟動每日排程
# 14:00=每日股票基本資料轉檔
print(f"{StockLib.getNowDatetime()}-scheduler 批次排程啟動-14:00=每日股票基本資料轉檔")
scheduler.add_job(getStockInfo, "cron", hour="14", minute="00", misfire_grace_time=3600)

# 14:30=每日取得盤後股價
print(f"{StockLib.getNowDatetime()}-scheduler 批次排程啟動-14:30=每日取得盤後股價")
scheduler.add_job(
    getStockAfter, "cron", hour="14", minute="30", misfire_grace_time=3600
)
